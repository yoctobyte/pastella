...
procedure PushTo(ni, peer: Integer; const m: TMsg);
var fd: Integer; begin
  ...
  SendFrame(fd, Encode(m)); PalClose(fd); end;

{ forward queued messages to both line neighbours }
procedure Forwarder(arg: Pointer);
var ni, i: Integer; m: TMsg; begin ni := Integer(PtrInt(arg));
  ...
  begin
    ...
    begin
      ...
      if ni > 0   then PushTo(ni, ni-1, m);
      if ni < K-1 then PushTo(ni, ni+1, m);
    end;
    CoSleep(40);
  end;
end;
...
procedure Originate(arg: Pointer);
