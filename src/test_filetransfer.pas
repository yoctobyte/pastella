...
var h: AnsiString; L: Integer; begin h := RecvExact(fd,4); if Length(h)<4 then begin RecvFrame := ''; Exit; end;
...
var v,fd,ni,i,nwant: Integer; phave,pwant,mywant,data: AnsiString; begin
  ...
  phave := RecvFrame(fd); SendFrame(fd,NHave(ni)); pwant := RecvFrame(fd); ServeWants(ni,fd,pwant);
  mywant := WantFrom(ni,phave); nwant := CountLines(mywant); SendFrame(fd,mywant);
  for i := 1 to nwant do begin data := RecvFrame(fd); NPut(ni,data); end; PalClose(fd); end;
...
var fd,i,nwant: Integer; phave,mywant,pwant,data: AnsiString; begin
  ...
  SendFrame(fd,NHave(ni)); phave := RecvFrame(fd);
  mywant := WantFrom(ni,phave); nwant := CountLines(mywant); SendFrame(fd,mywant);
  for i := 1 to nwant do begin data := RecvFrame(fd); NPut(ni,data); end;
  pwant := RecvFrame(fd); ServeWants(ni,fd,pwant); PalClose(fd); end;
...
procedure Neighbour(arg: Pointer);
var ni,r: Integer; begin ni := Integer(PtrInt(arg)); CoSleep(200);
  for r := 1 to ROUNDS do begin
    if ni > 0   then GossipOnce(ni, TBASE+ni-1);
    if ni < K-1 then GossipOnce(ni, TBASE+ni+1);
    CoSleep(120); end; end;
